<?php
/**
 * class-yivic-vertical-menu-settings.php
 *
 * @since: 1.1.0
 * @version: 1.1.0
 */
...
class YIVICFL_Vertical_Menu_Settings
{
    /**
    ...
    {
        add_options_page(
            __('YIVIC Flatsome Vertical Menu Settings', 'skype-live-chat'),
            __('YIVIC FS Vertical Menu', 'skype-live-chat'),
            'manage_options',
            'yivic-flatsome-vertical-menu',
    ...
    {
        $this->options = get_option('yivic_vm');

        $menu_event = isset($this->options['menu_event']) ? esc_attr($this->options['menu_event']) : 'click';

        ?>
        <div class="wrap">
            <h1><?php _e('YIVIC Flatsome Vertical Menu Settings', 'skype-live-chat'); ?></h1>

            <form method="post" action="options.php">
                <?php settings_fields('yivic_vm'); ?>
                <?php do_settings_sections('yivic_vm'); ?>
                <table class="form-table">
                    <tr valign="top">
                        <th scope="row"><?php _e('Menu Title', 'skype-live-chat'); ?></th>
                        <td>
                            <input type="text" name="yivic_vm[menu_title]" class="regular-text"
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <select class="regular-text" name="yivic_vm[menu_event]" id="menu_event">
    ...
    {
        register_setting(
            'yivic_vm', // Option group
            'yivic_vm', // Option name
...
if (is_admin()) {
    new YIVICFL_Vertical_Menu_Settings();
}