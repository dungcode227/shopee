<?php
/*
* Plugin Name: ManhPhuc - Quick Buy - Mua Hàng Nhanh
* Version: 2.0.0
* Description: ManhPhuc Quick Buy là plugin giúp khách hàng có thể mua nhanh sản phẩm ngay tại trang chi tiết dưới dạng popup
* Author: Nguyen Manh Phuc
* Author URI: http://yivic.com
* Plugin URI: http://yivic.com/san-pham/plugin-mua-hang-nhanh-cho-woocommerce-woocommerce-quick-buy/
* Text Domain: manhphuc-quickbuy
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
    if (!class_exists('ManhPhuc_Quick_Buy')) {
        class ManhPhuc_Quick_Buy
        {
            ...
            public $_defaultOptions = array(
                ...
                'popup_sucess' => '<div class="popup-message success" style="color:#333;"><p class="clearfix" style="font-size:22px;color: #00c700;text-align:center">Đặt hàng thành công!</p><p class="clearfix" style="color: #00c700;padding: 10px 0;">Mã đơn hàng <span style="color: #333;font-weight: bold">#%%order_id%%</span></p><p class="clearfix">ManhPhuc SHOP sẽ liên hệ với bạn trong 12h tới. Cám ơn bạn đã cho chúng tôi cơ hội được phục vụ.<br><strong>Hotline:</strong> 0936.307.069</p><p class="clearfix"><strong>Ghi chú: </strong>Đơn hàng chỉ có hiệu lực trong vòng 48h</p><div></div><div></div></div>',
            ...
            public function __construct()
            {
                ...
                add_filter( 'plugin_action_links_' . MANHPHUC_QB_BASENAME, array( $this, 'add_action_links' ), 10, 2 );
                ...
                if($quickbuy_settings['enable']){
                    ...
                    add_shortcode('manhphuc_quickbuy', array($this, 'manhphuc_button_quick_buy'));
                    ...
                    add_action( 'wp_ajax_manhphuc_quickbuy', array($this,'manhphuc_quickbuy_func') );
                    add_action( 'wp_ajax_nopriv_manhphuc_quickbuy', array($this,'manhphuc_quickbuy_func') );

                    add_action( 'wp_ajax_manhphuc_form_quickbuy', array($this,'manhphuc_form_quickbuy_func') );
                    add_action( 'wp_ajax_nopriv_manhphuc_form_quickbuy', array($this,'manhphuc_form_quickbuy_func') );
                    ...
                    add_action('manhphuc_prod_variable','woocommerce_template_single_add_to_cart');
                ...
                if( is_admin() ) {
                    add_action('in_plugin_update_message-' . MANHPHUC_QB_BASENAME, array($this,'manhphuc_modify_plugin_update_message'), 10, 2 );
                }
            ...
            {
                if (!defined('MANHPHUC_QB_VERSION_NUM'))
                    define('MANHPHUC_QB_VERSION_NUM', $this->_version);
                if (!defined('MANHPHUC_QB_URL'))
                    define('MANHPHUC_QB_URL', plugin_dir_url(__FILE__));
                if (!defined('MANHPHUC_QB_BASENAME'))
                    define('MANHPHUC_QB_BASENAME', plugin_basename(__FILE__));
                if (!defined('MANHPHUC_QB_PLUGIN_DIR'))
                    define('MANHPHUC_QB_PLUGIN_DIR', plugin_dir_path(__FILE__));
            }

            function dvls_load_textdomain() {
                load_textdomain('manhphuc-quickbuy', dirname(__FILE__) . '/languages/manhphuc-quickbuy-' . get_locale() . '.mo');
            }

            function manhphuc_button_quick_buy($atts)
            {
                ...
                if($this_product) {
                    ...
                    if (!is_admin() && $this_product->is_in_stock()):
                        if(!$small_link):
                        ?>
                        <a href="javascript:void(0);" class="manhphuc_buy_now manhphuc_buy_now_style" data-id="<?php echo $this_product->get_id(); ?>">
                        ...
                        ?>
                        <a href="javascript:void(0);" class="manhphuc_buy_now" data-id="<?php echo $this_product->get_id();?>"><?php echo ($button_text1) ? $button_text1 : $quickbuy_settings['button_text1']; ?></a>
                        <?php
                        endif;
                        if ($view) {
            ...
            function manhphuc_form_quickbuy_func(){
                ...
                if($args){
                    ...
                    if($prodID){
                        ...
                        if($thisProduct){
                            ...
                            ?>
                            <div class="manhphuc-popup-content-left <?php echo ($quickbuy_settings['popup_infor_enable']) ? '' : 'popup_quickbuy_hidden_mobile';?>">
                                <div class="manhphuc-popup-prod">
                                    <?php
                                    $thumbArgs = wp_get_attachment_image_src(get_post_thumbnail_id($thisProduct->get_id()), 'shop_thumbnail');
                                    if($thumbArgs):?>
                                        <div class="manhphuc-popup-img"><img src="<?php echo $thumbArgs[0]?>" alt=""/></div>
                                    <?php endif;?>
                                    <div class="manhphuc-popup-info">
                                        <span class="manhphuc_title"><?php echo $thisProduct->get_title();?></span>
                                        <?php if($thisProduct->get_type() == 'simple'):?><span class="manhphuc_price"><?php echo $thisProduct->get_price_html(); ?></span><?php endif;?>
                                    </div>
                                </div>
                                <div class="manhphuc_prod_variable" data-simpleprice="<?php echo $thisProduct->get_price();?>">
                                    <?php
                                    $product = $thisProduct;
                                    do_action('manhphuc_prod_variable');
                                    ?>
                                </div>
                                <?php echo $quickbuy_settings['popup_mess'];?>
                            </div>
                            <div class="manhphuc-popup-content-right">
                                <form class="manhphuc_cusstom_info" id="manhphuc_cusstom_info" method="post">
                                    <div class="popup-customer-info">
                                        <div class="popup-customer-info-title"><?php _e('Thông tin người mua','manhphuc-quickbuy')?></div>
                                        <?php do_action('before_field_manhphuc_quickbuy');?>
                                        ...
                                        if($vn_states && is_array($vn_states) && $quickbuy_settings['enable_location']):
                                            ...
                                            <div class="popup-customer-info-group">
                                                <div class="popup-customer-info-item-1">
                                                    <select class="customer-location" name="customer-location" id="manhphuc_city">
                                        ...
                                        <?php else:?>
                                            ...
                                            <div class="popup-customer-info-group">
                                                <div class="popup-customer-info-item-3-13">
                                                    <select class="customer-location" name="customer-location" id="manhphuc_city">
                                                        <option value=""><?php _e('Tỉnh thành','manhphuc')?></option>
                                                        <?php foreach($vn_states as $k=>$v):?>
                                                ...
                                                <div class="popup-customer-info-item-3-23">
                                                    <select class="customer-quan" name="customer-quan" id="manhphuc_district">
                                                ...
                                                <div class="popup-customer-info-item-3-33">
                                                    <select class="customer-xa" name="customer-xa" id="manhphuc_ward">
                                        ...
                                        <div class="popup-customer-info-group">
                                            <div class="popup-customer-info-item-1">
                                                <button type="button" class="manhphuc-order-btn">Đặt hàng ngay</button>
                                            </div>
                                        </div>
                                        <div class="popup-customer-info-group">
                                            <div class="popup-customer-info-item-1">
                                                <div class="manhphuc_quickbuy_mess"></div>
                                            </div>
                                        </div>
                                        <?php do_action('after_field_manhphuc_quickbuy');?>
                                    </div>
            ...
            function quick_buy_popup_content($args = array(), $view = true)
            {
                ...
                if($args){
                    ...
                    if($prodID){
                        ...
                        if($thisProduct){
                            ?>
                            <div class="manhphuc-popup-quickbuy" data-popup="popup-quickbuy" id="popup_content_<?php echo $thisProduct->get_id();?>">
                                <div class="manhphuc-popup-inner">
                                    <div class="manhphuc-popup-title">
                                        <span><?php printf($quickbuy_settings['popup_title'], $thisProduct->get_title());?></span>
                                        <button type="button" class="manhphuc-popup-close"></button>
                                    </div>
                                    <div class="manhphuc-popup-content manhphuc-popup-content_<?php echo $thisProduct->get_id();?> <?php echo (!$view) ? 'ghn_not_loaded' : '';?>">
                                        <?php if($view):?>
            ...
            function check_plugin_active($base_plugin = 'manhphuc-woo-address-selectbox/manhphuc-woo-address-selectbox.php'){
                if (
                    in_array( $base_plugin, apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ||
                    in_array( 'woo-vietnam-checkout/manhphuc-woo-address-selectbox.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ||
                    in_array( 'manhphuc-woo-ghtk/manhphuc-woo-ghtk.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )
                ) {
                    return true;
                }
                return false;
            ...
            function add_button_quick_buy(){
                global $product;
                echo do_shortcode('[manhphuc_quickbuy view="0" id="'.$product->get_id().'"]');
            }

            function add_quick_buy_to_loop_func(){
                global $product;
                echo do_shortcode('[manhphuc_quickbuy small_link="1" id="'.$product->get_id().'"]');
            }

            function manhphuc_get_rates($package = array(), $product_info = array()){
            ...
            function manhphuc_calculate_shipping($package = array(), $product_info = array()){
                $available_methods = $this->manhphuc_get_rates($package, $product_info);
            ...
            function load_diagioihanhchinh_func() {
                ...
                if(
                    ...
                    !class_exists('ManhPhuc_Woo_Vietnam_Checkout_Class') &&
                    !class_exists('ManhPhuc_Woo_GHTK_Class')
                ) wp_send_json_error();
                if(class_exists('Woo_Address_Selectbox_Class')){
                    $address = new Woo_Address_Selectbox_Class;
                }elseif(class_exists('ManhPhuc_Woo_Vietnam_Checkout_Class')){
                    $address = new ManhPhuc_Woo_Vietnam_Checkout_Class;
                }else{
                    $address = new ManhPhuc_Woo_GHTK_Class;
                }
                ...
                if($quickbuy_settings['enable_ship']) {
                    ...
                    $result['shipping'] = $this->manhphuc_calculate_shipping($package, $product_info);
                }
                if($getvalue == 1 && $matp){
            ...
            function manhphuc_quickbuy_func(){
                global $quickbuy_settings;
                $prod_id = isset($_POST['prod_id']) ? intval($_POST['prod_id']) : '';
            ...
            {
                if (strpos($file, 'manhphuc-quick-buy.php') !== false) {
                    $settings_link = '<a href="' . admin_url('options-general.php?page=quickkbuy-setting') . '" title="' . __('Settings', 'manhphuc-quickbuy') . '">' . __('Settings', 'manhphuc-quickbuy') . '</a>';
            ...
            {
                global $quickbuy_settings;
                wp_enqueue_style('manhphuc-quickbuy-style', plugins_url('css/manhphuc-quick-buy.css', __FILE__), array(), $this->_version, 'all');
                ...
                wp_enqueue_script('manhphuc-quickbuy-script', plugins_url('js/manhphuc-quick-buy.js', __FILE__), array('jquery','bpopup', 'wc-add-to-cart-variation'), $this->_version, true);
                $array = array(
                ...
                wp_localize_script('manhphuc-quickbuy-script', 'manhphuc_quickbuy_array', $array);
            }
            ...
            public function admin_enqueue_scripts()
            {
                ...
                if (isset($current_screen->base) && $current_screen->base == 'settings_page_quickkbuy-setting') {
                    wp_enqueue_style('manhphuc-quickbuy-admin-styles', plugins_url('/css/admin-style.css', __FILE__), array(), $this->_version, 'all');
                    wp_enqueue_script('manhphuc-quickbuy-admin-js', plugins_url('/js/admin-jquery.js', __FILE__), array('jquery'), $this->_version, true);
                }
            }
            ...
            {
                add_options_page(
                    __('Quick Buy Setting', 'manhphuc-quickbuy'),
                    __('Quick Buy Setting', 'manhphuc-quickbuy'),
                    'manage_options',
                    'quickkbuy-setting',
                    array(
                        $this,
                        'manhphuc_settings_page'
                    )
                );
            }
            ...
            function manhphuc_modify_plugin_update_message( $plugin_data, $response ) {
                global $quickbuy_settings;
                ...
                echo '<br />' . sprintf( __('<strong>Mua bản quyền để được tự động update. <a href="%s" target="_blank">Xem thêm thông tin mua bản quyền</a></strong> hoặc liên hệ mua trực tiếp qua <a href="%s" target="_blank">facebook</a>', 'manhphuc-quickbuy'), $PluginURI, 'http://m.me/manhphucofficial');
            }

            function manhphuc_settings_page()
            {
                global $quickbuy_settings;
                ?>
                <div class="wrap manhphuc_quickbuy">
                    <h1><?php _e('Quick Buy Setting', 'manhphuc-quickbuy');?></h1>
                    <form method="post" action="options.php" novalidate="novalidate">
                        <?php settings_fields($this->_optionGroup); ?>
                        <h2><?php _e('General settings','manhphuc-quickbuy');?></h2>
                        <table class="form-table">
                            <tbody>
                            <tr>
                                <th scope="row"><label for="enable"><?php _e('Enable','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label>
                                        <input type="radio" id="enable" value="1" <?php checked(1,$quickbuy_settings['enable']);?> name="<?php echo $this->_optionName . '[enable]'?>"> <?php _e('Active','manhphuc-quickbuy');?>
                                    </label>
                                    <label>
                                        <input type="radio" id="enable" value="0" <?php checked(0,$quickbuy_settings['enable']);?> name="<?php echo $this->_optionName . '[enable]'?>"> <?php _e('Deactive','manhphuc-quickbuy');?>
                                    </label>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="enable_location"><?php _e('Select location','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label>
                                        <input type="checkbox" id="enable_location" value="1" <?php checked(1,$quickbuy_settings['enable_location']);?> name="<?php echo $this->_optionName . '[enable_location]'?>"> <?php _e('Active','manhphuc-quickbuy');?><br>
                                        <small><?php _e('Requires have state','manhphuc-quickbuy');?></small>
                                    </label>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="enable_ship"><?php _e('Shipping','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label><input type="checkbox" id="enable_ship" value="1" <?php checked(1,$quickbuy_settings['enable_ship']);?> name="<?php echo $this->_optionName . '[enable_ship]'?>"> <?php _e('Active','manhphuc-quickbuy');?></label>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="require_email"><?php _e('Require Email','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label><input type="checkbox" id="require_email" value="1" <?php checked(1,$quickbuy_settings['require_email']);?> name="<?php echo $this->_optionName . '[require_email]'?>"> <?php _e('Active','manhphuc-quickbuy');?></label>
                                </td>
                            </tr>
                            <?php if($this->check_plugin_active()):?>
                                <tr>
                                    <th scope="row"><label for="require_district"><?php _e('Require District','manhphuc-quickbuy');?></label></th>
                                    <td>
                                        <label><input type="checkbox" id="require_district" value="1" <?php checked(1,$quickbuy_settings['require_district']);?> name="<?php echo $this->_optionName . '[require_district]'?>"> <?php _e('Require','manhphuc-quickbuy');?></label>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row"><label for="require_village"><?php _e('Require Village','manhphuc-quickbuy');?></label></th>
                                    <td>
                                        <label><input type="checkbox" id="require_village" value="1" <?php checked(1,$quickbuy_settings['require_village']);?> name="<?php echo $this->_optionName . '[require_village]'?>"> <?php _e('Require','manhphuc-quickbuy');?></label>
                                    </td>
                                </tr>
                            <?php endif;?>
                            <tr>
                                <th scope="row"><label for="require_address"><?php _e('Require Address box','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label><input type="checkbox" id="require_address" value="1" <?php checked(1,$quickbuy_settings['require_address']);?> name="<?php echo $this->_optionName . '[require_address]'?>"> <?php _e('Require','manhphuc-quickbuy');?></label>
                                </td>
                            </tr>
                            </tbody>
                        </table>
                        <h2><?php _e('Button quick buy','manhphuc-quickbuy');?></h2>
                        <table class="form-table">
                            <tbody>
                            <tr>
                                <th scope="row"><label for="in_loop_prod"><?php _e('Hiển thị trong list sản phẩm','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label><input type="checkbox" id="in_loop_prod" value="1" <?php checked(1,$quickbuy_settings['in_loop_prod']);?> name="<?php echo $this->_optionName . '[in_loop_prod]'?>"> <?php _e('Active','manhphuc-quickbuy');?></label>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="button_text1"><?php _e('Button text','manhphuc-quickbuy');?></label></th>
                                <td>
                            ...
                            <tr>
                                <th scope="row"><label for="button_text2"><?php _e('Button sub text','manhphuc-quickbuy');?></label></th>
                                <td>
                        ...
                        <h2><?php _e('Popup setting','manhphuc-quickbuy');?></h2>
                        <table class="form-table">
                            <tbody>
                            <tr>
                                <th scope="row"><label for="popup_infor_enable"><?php _e('View product info on mobile','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label><input type="checkbox" id="popup_infor_enable" value="1" <?php checked(1,$quickbuy_settings['popup_infor_enable']);?> name="<?php echo $this->_optionName . '[popup_infor_enable]'?>"> <?php _e('Active','manhphuc-quickbuy');?></label>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="popup_title"><?php _e('Popup title','manhphuc-quickbuy');?></label></th>
                                <td>
                                    ...
                                    <br><small><?php _e('%s to view product title','manhphuc-quickbuy');?></small>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="popup_mess"><?php _e('Popup mess','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <?php
                                    $settings = array(
                            ...
                            <tr>
                                <th scope="row"><label for="popup_gotothankyou"><?php _e('Go to thank you page','manhphuc-quickbuy');?></label></th>
                                <td>
                                    <label><input type="checkbox" id="popup_gotothankyou" value="1" <?php checked(1,$quickbuy_settings['popup_gotothankyou']);?> name="<?php echo $this->_optionName . '[popup_gotothankyou]'?>"> <?php _e('Active','manhphuc-quickbuy');?></label>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="popup_sucess"><?php _e('Checkout successfully mess','manhphuc-quickbuy');?></label><br>
                                    <small><?php _e('Type %%order_id%% to view Order ID','manhphuc-quickbuy')?></small></th>
                                <td>
                                    <?php
                                    $settings = array(
                            ...
                            <tr>
                                <th scope="row"><label for="popup_error"><?php _e('Checkout error message','manhphuc-quickbuy');?></label></th>
                                <td>
                            ...
                            <tr>
                                <th scope="row"><label for="out_of_stock_mess"><?php _e('Out of stock message','manhphuc-quickbuy');?></label></th>
                                <td>
                        ...
                        <h2><?php _e('License','manhphuc-quickbuy');?></h2>
                        <table class="form-table">
                            <tbody>
                                <tr>
                                    <th scope="row"><label for="license_key"><?php _e('License key','manhphuc-quickbuy');?></label></th>
                                    <td>
                                        ...
                                        <?php if(!$quickbuy_settings['license_key']):?><br><small><?php echo sprintf( __('<strong>Gửi email + domain qua <a href="%s" target="_blank">facebook</a> để nhận license', 'manhphuc-quickbuy'), 'http://m.me/manhphucofficial');?></small><?php endif;?>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
